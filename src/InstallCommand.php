...
class InstallCommand extends Command
{
   ...
   {
      // It's a long story, but yes, you do indeed need to do
      // two separate transformations here.
      $content = str_replace("assets/", "{{dest}}", str_replace("src/", "{{source}}", $content));
      $content = str_replace('{{dest}}', rtrim($this->compiledPath, '/').'/', str_replace('{{source}}', rtrim($this->srcPath,'/').'/', $content));
   ...
   private function copyFiles($list, $dlPath)
   {
      foreach ($list as $file) {
         if ($file['type'] === 'file') {
            ...
            $newPath = str_replace("src", rtrim($this->srcPath, '/'), $newPath);
